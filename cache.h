
#ifndef CACHE_H
#define CACHE_H

#include <cmath>
#include <iostream>

typedef unsigned long ulong;
typedef unsigned char uchar;
typedef unsigned int uint;

/****add new states, based on the protocol****/
enum {
	INVALID = 0, 
	EXCLUSIVE, 
	SHARED, 
	MODIFIED,
    EXCLUSIVE_DRAGON,
	SHARED_CLEAN_DRAGON,
	SHARED_MODIFIED_DRAGON,
	MODIFIED_DRAGON
};

class cacheLine {
protected:
	ulong tag;
	ulong Flags;   // 0:invalid, 1:valid, 2:dirty
	ulong seq;

public:
	cacheLine() {
		tag = 0;
		Flags = 0;
		seq = 0;
	}
	ulong getTag() {
		return tag;
	}
	ulong getFlags() {
		return Flags;
	}
	ulong getSeq() {
		return seq;
	}
	void setSeq(ulong Seq) {
		seq = Seq;
	}
	void setFlags(ulong flags) {
		Flags = flags;
	}
	void setTag(ulong a) {
		tag = a;
	}
	void invalidate() {
		tag = 0;
		Flags = INVALID;
	} 
	bool isValid() {
		return ((Flags) != INVALID);
	}
};

class Cache {
protected:
	ulong size, lineSize, assoc, sets, log2Sets, log2Blk, tagMask, numLines;
	ulong reads, readMisses, writes, writeMisses, writeBacks;

public:
	int cache2cache, interventions,mem_transac,invalidations, flushes, protocol;

	//******///
	//add coherence counters here///
	//******///

	cacheLine **cache;
	ulong calcTag(ulong addr) {
		return (addr >> (log2Blk));
	}
	ulong calcIndex(ulong addr) {
		return ((addr >> log2Blk) & tagMask);
	}
	ulong calcAddr4Tag(ulong tag) {
		return (tag << (log2Blk));
	}

public:
	ulong currentCycle;

	Cache(int, int, int, int);
	~Cache() {
		delete cache;
	}

	cacheLine *findLineToReplace(ulong addr);
	cacheLine *fillLine(ulong addr);
	cacheLine * findLine(ulong addr);
	cacheLine * getLRU(ulong);

	ulong getRM() {
		return readMisses;
	}
	ulong getWM() {
		return writeMisses;
	}
	ulong getReads() {
		return reads;
	}
	ulong getWrites() {
		return writes;
	}
	ulong getWB() {
		return writeBacks;
	}

	ulong getMT(){
		return mem_transac;
	}

	float getMR() {
	      float a = readMisses + writeMisses;
	      float b = reads + writes;
	      float c = (a/b)*100;
	      return c;
	}

	void writeBack(ulong) {
		writeBacks++;
	}
	void Access(ulong, uchar, ulong);
	void printStats();
	void updateLRU(cacheLine *);
	int busRd(ulong);
	int busRdx(ulong);
	void busUpgr(ulong);
	int busUpdate(ulong);
	//******///
	//add other functions to handle bus transactions///
	//******///

};

#endif
